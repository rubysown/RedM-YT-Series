...
namespace Client
{
    class Util
    {
        public static List<string> horseModels = new List<string>
        {
        ...
        public static Vector3 GetPedCoords(int pedId, int forwardVector = 0)
        {
            Vector3 pos = Function.Call<Vector3>(Hash.GET_ENTITY_COORDS, pedId);
            if(forwardVector != 0)
            {
                Vector3 forward = Function.Call<Vector3>(Hash.GET_ENTITY_FORWARD_VECTOR, pedId);
                pos += (forward * forwardVector);
            }
            return pos;
        }
        public static Vector3 GetEntityForwardVector(int pedId, int forwardVector = 1)
        {
            Vector3 forward = Function.Call<Vector3>(Hash.GET_ENTITY_FORWARD_VECTOR, pedId);
            return forward * forwardVector;
        }
        public static void GiveWeaponToPed(int ped, WeaponHash weapon)
        {
